...
You can delete a file by passing its URL to the `deleteFile` method.
...
Use `deleteFiles` to send one request for multiple files. Storage failures are
reported per URL:

```tsx
const result = await edgestore.publicFiles.deleteFiles({
...
To confirm several temporary files in one request, use `confirmUploads`:

```tsx
const result = await edgestore.publicFiles.confirmUploads({
