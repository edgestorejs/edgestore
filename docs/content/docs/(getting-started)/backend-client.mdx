...
supports `upload`, `get`, `delete`, `deleteMany`, and private-read
`createSignedUrl` / `createSignedUrls`, accepting key or URL references.
Azure Blob Storage supports lookup, deletion, and private signed reads.
Unsupported operations, such as S3 listing and confirmation or Azure backend
upload, remain absent from each provider's client type.
