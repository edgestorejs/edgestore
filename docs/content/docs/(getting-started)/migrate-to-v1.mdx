...
If you used an earlier v1 prerelease, remove `createEdgeStore` and
`ConfiguredEdgeStore`. Use `router.client` for backend operations and pass
`{ router }` to your handler instead of `{ edgestore: configuredEdgeStore }`.
Move provider configuration to `.provider(...)` and any `baseUrl` option to the
second argument of `es.router(buckets, { baseUrl })`.
...
| `initEdgeStoreClient`                       | `router.client`      | Read the client from the configured hosted provider.       |
