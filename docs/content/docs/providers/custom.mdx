...
edgeStore.client.files.get({ key: 'files/report.pdf' });
```
...
The backend client uses bucket-scoped names: `get`, `list`, `listAll`,
`confirm`, `confirmMany`, `delete`, `deleteMany`, `restore`, `restoreMany`,
