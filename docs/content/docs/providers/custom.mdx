...
completion routes. Providers may also implement `uploads.multipart.abort` and
return `multipart.abortSupported: true` to enable cleanup after a failed or
canceled browser transfer. Multipart operations must validate an upload session
scoped to its authorized object; do not trust a client-supplied key or raw upload
ID alone.

Single-part upload plans can include `uploadHeaders` for provider-required PUT
headers and `key` for a stable object reference. Set `disableDevProxy: true` on
a direct-storage provider to keep canonical URLs in development instead of
using the hosted provider's cookie proxy.
