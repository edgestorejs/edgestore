...
- Stage 3a: [draft PR #240](https://github.com/edgestorejs/edgestore/pull/240), stacked on #239. Passed 18 new tests, all 256 CLI tests, CLI typecheck/lint/build, and a built-command smoke test against TanStack Start. Resolution is bounded to the application's repository/workspace and follows package-manager symlinks, never ambient global modules. Agent/MCP inspection remains explicitly not-inspected until stage 4.
- Stage 3b: [draft PR #241](https://github.com/edgestorejs/edgestore/pull/241), stacked on #240. Passed 59 focused tests, all 266 CLI tests, CLI typecheck/lint/build. Automated project-key commands require protected file delivery; structured results omit secrets. Init inspects configured/existing env destinations before defaulting. Symlink escape checks and secret-sentinel tests included. Breaking CLI changeset documents the structured-output migration.
- Stage 4a: [draft PR #242](https://github.com/edgestorejs/edgestore/pull/242), stacked on #241. Independent MCP configuration for Codex, Claude Code, and Cursor. All 296 CLI tests, typecheck/lint/build, scoped format, dependency policy and command-help smoke passed. Authentication deferred; hosted scope consent remains an integration prerequisite, not a claimed read-only grant.
- Stage 5a: [draft PR #243](https://github.com/edgestorejs/edgestore/pull/243), stacked on #242 and moved before skill distribution. Read-only/offline doctor and shared installation-role detection passed 309 CLI tests, typecheck/lint/build, dependency policy and the real Start offline smoke. A follow-up regression test keeps symlinked worktree env files explicitly skipped/uninspected. Source checks are bounded AST observations, not a complete module graph. Runtime mounting/provider ancestry, custom env loading and remote bucket mappings remain explicitly unverified for application-path evaluation.
- Stage 5b: [draft PR #244](https://github.com/edgestorejs/edgestore/pull/244), stacked on #243. Canonical setup skill and repository skill-only plugin passed both validators and `skills@1.5.23 add ./skills --list` (telemetry disabled, no installation). No registered hosted connector ID found, so `.app.json` remains an explicit release prerequisite. New-grant read-only defaults verified in sibling OAuth source/tests; live consent pending. Fixed MCP discovery to avoid treating a plugin name as proof of a hosted connection. Behavioral evaluation follows the packed installer.
- Stage 4b: [draft PR #245](https://github.com/edgestorejs/edgestore/pull/245), stacked on #244. All 335 CLI tests, CLI typecheck/lint/build, generator typecheck/lint, docs contract tests, deterministic asset check and exact canonical-byte real-tarball check passed. Skills and MCP have separate ownership files; both plans preflight before writes. Context now reports per-client visible configurations. Existing React Router/Astro full-stack classification preserved.
- Provisioning safety follow-up: [draft PR #246](https://github.com/edgestorejs/edgestore/pull/246), stacked on #245. Live-test preparation identified raw initial-project-key and management-token fields still present in JSON. Automated project creation now requires `--without-key`; token creation uses protected `--output`. JSON omits both secrets; interactive one-time human display is preserved. All 343 CLI tests, typecheck/lint/build passed. GitHub CI is green through this stack, including CLI smoke tests on Linux, macOS and Windows.
- Stage 6a: [draft PR #247](https://github.com/edgestorejs/edgestore/pull/247), stacked on #246. Five policy/fixture tests and harness typecheck pass. All three golden-path tarball installations pass package resolution/version/export checks, selected adapter imports, bundled reference indexes, idempotent skill installation, three-client MCP setup/status/removal, offline doctor, and starter build/typecheck. Matched baseline/skill workspaces and prompts are prepared but no coding-agent evaluation has run. The harness does not yet automate live provisioning, browser upload, or ledger cleanup. Final local package builds/typechecks/runtime tests/type tests, dependency policy and repository format checks all pass.
- Stages 6b–7: incomplete. Dev targets identified from sibling dev environment: API `api-dev.edgestore.dev`, files `files-dev.edgestore.dev`; API health and MCP protected-resource metadata respond successfully. Local MySQL port 3300 is running, local API/dashboard are not. No credential values have been emitted. Dedicated non-production account/management access is still needed for scoped live testing. Hosted plugin connector registration and actual OAuth consent remain external prerequisites; the skill-only plugin and direct MCP setup are independently implemented.
