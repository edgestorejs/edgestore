...
- Provisioning safety follow-up: live-test preparation identified raw initial-project-key and management-token fields still present in JSON. Automated project creation now requires `--without-key`; token creation uses protected `--output`. JSON omits both secrets; interactive one-time human display is preserved. All 343 CLI tests, typecheck/lint/build passed. Draft publication in progress.
- Stages 6–7: pending. Dev targets identified from sibling dev environment: API `api-dev.edgestore.dev`, files `files-dev.edgestore.dev`; local MySQL port 3300 is running, local API/dashboard are not. No credential values have been emitted. Non-production account/control-plane access and live upload paths still need validation.
