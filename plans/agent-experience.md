...
- Stage 6a: manual fixture/artifact harness implemented. Five policy/fixture tests and harness typecheck pass. All three golden-path tarball installations pass package resolution/version/export checks, selected adapter imports, bundled reference indexes, idempotent skill installation, three-client MCP setup/status/removal, offline doctor, and starter build/typecheck. Matched baseline/skill workspaces and prompts are prepared but no coding-agent evaluation has run. The harness does not yet automate live provisioning, browser upload, or ledger cleanup. Draft publication in progress.
