...
- Stage 5b: canonical setup skill and repository skill-only plugin implemented; both skill/plugin validators and `skills@1.5.23 add ./skills --list` passed (telemetry disabled, no installation). No registered hosted connector ID found, so `.app.json` remains an explicit release prerequisite. New-grant read-only defaults verified in sibling OAuth source/tests; live consent pending. Fixed MCP discovery to avoid treating a plugin name as proof of a hosted connection. Draft publication in progress; behavioral evaluation follows the packed installer.
- Stages 4b–7: pending.
